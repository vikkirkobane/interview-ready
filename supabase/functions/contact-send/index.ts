...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Dispatch notification email to company support email
    ...
    if (resendApiKey) {
      ...
      const resendResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: `Interview Ready <info@appinterviewready.top>`,
      ...
      // Send auto-acknowledgement to sender
      try {
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: `Interview Ready Support <info@appinterviewready.top>`,
            to: [email.trim()],
        ...
        console.warn('Auto-reply error:', ackError);
      }
    }
